...
class SdeSolver:
    ...
    """
    # TODO: Adapt to general time grid use
    @staticmethod
        ...
        if orderDimensions is not None:
            for i in range(1, nSteps):
                for dimension in orderDimensions:
                    x[i, dimension] = (x[i - 1, dimension] + drift[dimension](timeGrid[i - 1], x[i - 1]) *
                                       (timeGrid[i] - timeGrid[i - 1]) + diffusion[dimension](timeGrid[i - 1], x[i - 1])
                                       * (bb[i] - bb[i - 1]))
        else:
            for i in range(1, nSteps):
                x[i] = (x[i - 1] + drift(timeGrid[i - 1], x[i - 1]) * (timeGrid[i] - timeGrid[i - 1])
    ...
    def absoluteEuler(timeGridInstance: TimeGrid, nSteps: int, startingPoint: np.array,
        ...
        x[0] = startingPoint  # initial condition
        for i in range(1, nSteps):
            x[i] = x[i - 1] + np.abs(drift(timeGrid[i - 1], x[i - 1]) * (timeGrid[i] - timeGrid[i-1])+ diffusion(timeGrid[i - 1], x[i - 1])
                                     * (bb[i] - bb[i - 1]))
        return x
