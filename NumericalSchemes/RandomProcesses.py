...
class RandomProcesses:
    ...
    @staticmethod
    def ornsteinUhlenbeckExact(timeGridInstance: TimeGrid, nSteps: int, startingPoint: float, theta: float, mu: float,
                            sigma: float, seed: int = None) -> np.array:
        """
        Create a one dimensional Ornstein-Uhlenbeck process path
        @param timeGridInstance:
        @param nSteps:
        @param startingPoint:
        @param theta:
        ...
        if seed is not None:
            np.random.seed(seed)
        timeGrid = timeGridInstance.getTimeGrid(nSteps)
        ou = np.zeros((nSteps, 1))
        ou[0] = startingPoint
        for i in range(0, nSteps):
            expectation = startingPoint*np.exp(-theta*timeGrid[i]) + mu*(1-np.exp(-theta*timeGrid[i]))
            variance = (sigma**2)/(2*theta)*(1-np.exp(-2*theta*timeGrid[i]))
            ou[i] = expectation + np.sqrt(variance)*np.random.normal()
        return ou

    @staticmethod
    def ornsteinUhlenbeckExactPaths(timeGridInstance: TimeGrid, nPaths: int, nSteps: int, startingPoint: float,
                                    theta: float, mu: float, sigma: float, seed: int = None) -> np.array:
        """
        Create multiple Ornstein-Uhlenbeck process paths
        @param timeGridInstance:
        @param nPaths:
        @param nSteps:
        @param startingPoint:
        ...
        ouPaths = {}
        for numberPath in range(nPaths):
            ouPaths[numberPath] = RandomProcesses.ornsteinUhlenbeckExact(timeGridInstance, nSteps, startingPoint, theta,
                                                                        mu, sigma, seed)
        return ouPaths